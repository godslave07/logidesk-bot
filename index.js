...
Поля: from, fromCountry, to, toCountry, cargoName, weight (тонни), volume (м³), dateFrom (DD.MM.YYYY), dateTo (DD.MM.YYYY), truckType (Тент/Рефрижератор/Відкритий/Контейнер/Борт/Цистерна/Самоскид/Зерновоз/Автовоз/Критий), loadType (Повна/Часткова), price (число), currency (EUR/USD/UAH), paymentType (Готівка/Безнал/Картка), paymentMoment (після розвантаження/після завантаження/передоплата/часткова передоплата — витягуй з тексту коли/як відбувається оплата), phone, notes (ТІЛЬКИ якщо є реальна примітка про вантаж — НЕ про оплату; інакше порожній рядок). Якщо поле не знайдено — порожній рядок. ТІЛЬКИ JSON.`,
